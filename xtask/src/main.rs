...
fn sign_packages(target: &&str, boot_ver: &&str, updt_ver: &&str) -> Result<(), anyhow::Error> {
    ...
    match *target {
        ...
        "stm32h743" => {
            let _p = xshell::pushd(root_dir().join("boards/sign_images/signed_images"))?;
            cmd!("rust-objcopy -I elf32-littlearm ../../target/thumbv7em-none-eabihf/release/stm32h743_bootfw -O binary stm32h743_bootfw.bin").run()?;
            cmd!("rust-objcopy -I elf32-littlearm ../../target/thumbv7em-none-eabihf/release/stm32h743_updtfw -O binary stm32h743_updtfw.bin").run()?;

            let _p = xshell::pushd(root_dir().join("rbsigner"))?;
            cmd!("cargo run mcu-image ../boards/sign_images/signed_images/stm32h743_bootfw.bin nistp256 ../boards/sign_images/keygen/ecc256.der {boot_ver}").run()?;
            cmd!("cargo run mcu-image ../boards/sign_images/signed_images/stm32h743_updtfw.bin nistp256 ../boards/sign_images/keygen/ecc256.der {updt_ver}").run()?;
            Ok(())
        }
        "stm32f746" => {
            let _p = xshell::pushd(root_dir().join("boards/sign_images/signed_images"))?;
...
fn flash_signed_fwimages(target: &&str, boot_ver: &&str, updt_ver: &&str) -> Result<(), anyhow::Error> {
    match *target {
        ...
        "stm32h743" => {
            let _p = xshell::pushd(root_dir().join("boards/sign_images/signed_images"))?;
            let boot_part_addr = format!("0x{:x}", BOOT_PARTITION_ADDRESS);
            cmd!("probe-rs-cli download --format Bin --base-address {boot_part_addr} --chip STM32H743ZITx stm32h743_bootfw_v{boot_ver}_signed.bin").run()?;

            let updt_part_addr = format!("0x{:x}", UPDATE_PARTITION_ADDRESS);
            cmd!("probe-rs-cli download --format Bin --base-address {updt_part_addr} --chip STM32H743ZITx stm32h743_updtfw_v{updt_ver}_signed.bin").run()?;
            Ok(())
        }
        "stm32f746" => {
            let _p = xshell::pushd(root_dir().join("boards/sign_images/signed_images"))?;
...
fn flash_rustBoot(target: &&str) -> Result<(), anyhow::Error> {
    match *target {
        ...
        "stm32h743" => {
            let _p = xshell::pushd(root_dir().join("boards/bootloaders").join(target))?;
            cmd!("cargo flash --chip STM32H743ZITx --release").run()?;
            Ok(())
        }
        "stm32f746" => {
            let _p = xshell::pushd(root_dir().join("boards/bootloaders").join(target))?;
...
fn full_image_flash(target: &&str, boot_ver: &&str, updt_ver: &&str) -> Result<(), anyhow::Error> {
    match *target {
        ...
        "stm32h743" => {
            build_rustBoot(target)?;
            sign_packages(target, boot_ver, updt_ver)?;
            cmd!("probe-rs-cli erase --chip STM32H743ZITx").run()?;
            flash_signed_fwimages(target, boot_ver, updt_ver)?;
            flash_rustBoot(target)?;
            Ok(())
        }
...
fn erase_and_flash_trailer_magic(target: &&str) -> Result<(), anyhow::Error> {
    match *target {
        ...
        "stm32h743" => {
            let _p = xshell::pushd(root_dir().join("boards/sign_images/signed_images"))?;
            // just to ensure that an existing bootloader doesnt start to boot automatically - during a test
            cmd!("pyocd erase -t stm32h743 -s 0x0").run()?;
            let boot_trailer_magic = format!("0x{:x}", BOOT_PARTITION_ADDRESS + PARTITION_SIZE - 4);
            cmd!("pyocd erase -t stm32h743 -s {boot_trailer_magic}").run()?;
            cmd!("pyocd flash -t stm32h743 --base-address {boot_trailer_magic} trailer_magic.bin")
                .run()?;

            let updt_trailer_magic =
                format!("0x{:x}", UPDATE_PARTITION_ADDRESS + PARTITION_SIZE - 4);
            cmd!("pyocd erase -t stm32h743 -s {updt_trailer_magic}").run()?;
            cmd!("pyocd flash -t stm32h743 --base-address {updt_trailer_magic} trailer_magic.bin")
                .run()?;
            Ok(())
        }
        "stm32f746" => {
