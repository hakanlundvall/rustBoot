//! Flash  Read, Write and Erase operation for `stm32h743zitx`.
...
use stm32h743zitx_constants::*;

#[rustfmt::skip]
mod stm32h743zitx_constants {
    ...
    pub const PARTITION_SIZE            : u32 = 0x60000;
    pub const PARTITION_BOOT_ADDRESS    : u32 = 0x0802_0000;
    pub const PARTITION_UPDATE_ADDRESS  : u32 = 0x0812_0000;
    ...
    pub const STACK_UP        : u32 = 0x24_080_000;
    pub const RB_HDR_SIZE     : u32 = 0x100;
    pub const BASE_ADDR       : u32 = 0x08020000;
    pub const VTR_TABLE_SIZE  : u32 = 0x100;
    pub const FW_RESET_VTR    : u32 = BASE_ADDR + RB_HDR_SIZE + VTR_TABLE_SIZE + 0x199;
...
impl FlashInterface for FlashWriterEraser {
    ...
    fn hal_flash_write(&self, addr: usize, data: *const u8, len: usize) {
        ...
        let flash_write = |bank: &hal::device::flash::BANK, addr: usize, data: *const u8, len: usize| {
            let mut i = 0u32;
            let mut ii = 0u32;
            ...
            while i < len as u32 {
                ...
                // && (((((addr as u32) + i) & 0x1F) == 0) && (((data as u32) + i) & 0x1F) == 0))
                {
                    ...
                    while bank.sr.read().bsy().bit_is_set() {}
    
                    // Unlock the FLASH_CR register.
                    self.hal_flash_unlock();
    
                    // Flash clear errors
                    bank.sr.modify(|_, w| w.wrperr().clear_bit());
                    bank.sr.modify(|_, w| w.pgserr().clear_bit());
                    bank.sr.modify(|_, w| w.strberr().clear_bit());
                    bank.sr.modify(|_, w| w.incerr().clear_bit());
                    bank.sr.modify(|_, w| w.operr().clear_bit());
                    bank.sr.modify(|_, w| w.rdperr().clear_bit());
                    bank.sr.modify(|_, w| w.rdserr().clear_bit());
                    bank.sr.modify(|_, w| w.sneccerr1().clear_bit());
                    bank.sr.modify(|_, w| w.dbeccerr().clear_bit());
    
                    // Enable write operations by setting the PG bit in the FLASH_CR register.
                    bank.cr.modify(|_, w| unsafe {
                ...
                while bank.sr.read().qw().bit_is_set() {}
                ...
                if bank.sr.read().eop().bit_is_set() {
                    bank.sr.modify(|_, w| w.eop().set_bit()); // Clear
                }
        ...
        match addr {
            (0x0800_0000..=0x080F_FFFF) => flash_write(&self.nvm.bank1(), addr, data, len),
            (0x0810_0000..=0x081F_FFFF) => flash_write(&self.nvm.bank2(), addr, data, len),
    ...
    fn hal_flash_erase(&self, addr: usize, len: usize) {
        ...
        match address {
            ...
            (0x0810_0000..=0x0811_FFFF) => sec = 0,
            (0x0812_0000..=0x0813_FFFF) => sec = 1,
            (0x0814_0000..=0x0815_FFFF) => sec = 2,
            (0x0816_0000..=0x0817_FFFF) => sec = 3,
            (0x0818_0000..=0x0819_FFFF) => sec = 4,
            (0x081A_0000..=0x081B_FFFF) => sec = 5,
            (0x081C_0000..=0x081D_FFFF) => sec = 6,
            (0x081E_0000..=0x081F_FFFF) => sec = 7,
            _ => flag = false,
        }

        let binding = self.nvm.bank2();
        match address {
            (0x0800_0000..=0x080F_FFFF) => bank = &bank,
            (0x0810_0000..=0x081F_FFFF) => bank = &binding,
            _ => panic!("Invalid address"),
        }
        ...
        if flag {
            ...
            bank.cr.modify(|_, w| w.start().bit(true));
    ...
    fn hal_flash_lock(&self) {
        ...
        self.nvm.bank2().cr.modify(|_, w| w.lock().set_bit());
    }
...
pub fn boot_from(fw_base_address: usize) -> ! {
       ...
       unsafe {
        ...
        #[cfg(feature = "enforce_reset_vtr")]
        {
            rv = RefinedUsize::<0, 0, FW_RESET_VTR>::single_valued_int(
                *((fw_base_address + 4) as *const u32)).0;
        }
